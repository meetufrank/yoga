...
<html lang="en">
	<head>
		...
		<!-- text fonts -->
		<link rel="stylesheet" href="/yoga/Public/assets/css/ace-fonts.css" />

		<!-- ace styles -->
		<link rel="stylesheet" href="/yoga/Public/assets/css/ace.css" />

		<!--[if lte IE 9]>
			<link rel="stylesheet" href="/yoga/Public/assets/css/ace-part2.css" />
		<![endif]-->
		<link rel="stylesheet" href="/yoga/Public/assets/css/ace-rtl.css" />
...
.loginnone{display:none;}
	...
	<body class="login-layout light-login">
						...
						<div class="login-container">
							<div class="center">
								<h1>
									...
									<span class="red">生活的艺术后台管理控制台</span>
									<span  class="blue" style="font-family:microsoft yahei" id="id-text2"></span>
								</h1>
							</div>
												...
												<fieldset>
													<label class="block clearfix">
														<span class="block input-icon input-icon-right">
															<input type="text" class="form-control" name="admin_username" id="admin_username" placeholder="用户名"  value="<?php echo (session('remember_user')); ?>" />
													...
													<label class="block clearfix">
														<span class="block input-icon input-icon-right">
															<input type="password" class="form-control" name="admin_pwd" id="admin_pwd" placeholder="输入密码" value="<?php echo (session('remember_password')); ?>" />
													...
													<div class="clearfix">
														<label class="inline">
															<input type="checkbox" class="ace" name="remember" value="1" <?php if($Think.session.remember_user): ?>checked="checked"<?php endif; ?>/>
		...
		<script type="text/javascript">
			window.jQuery || document.write("<script src='/yoga/Public/assets/js/jquery.js'>"+"<"+"/script>");
		</script>
...
<![endif]-->
		<script type="text/javascript">
			if('ontouchstart' in document.documentElement) document.write("<script src='/yoga/Public/assets/js/jquery.mobile.custom.js'>"+"<"+"/script>");
		</script>
...
<script src="/yoga/Public/mobile/device.js"></script>

<script>
 if(device.mobile()){
        window.__url3__ = "<?php echo U('Mobile/index');?>";  //手机站地址
    }else{
    	window.__url3__ = "<?php echo U('Index/index');?>";
    }
//登入
$(function(){
	$('#runlogin').ajaxForm({
...
$(function(){
	...
	function checkFormreg(){
		if( '' == $.trim($('#admin_email').val())){
			layer.alert('注册邮箱不能为空', {icon: 6}, function(index){
 			layer.close(index);
			$('#admin_email').focus();
			});
			return false;
		...
		if($.trim($('#admin_pwdreg').val()) != $.trim($('#admin_newpwdreg').val())){
			layer.alert('2次输入密码不同，请重新输入', {icon: 6}, function(index){
 			layer.close(index);
			$('#admin_pwdreg').focus();
			});
			return false;
