      ...
      <script type="text/javascript" src="/yoga/Public/sldate/daterangepicker.js"></script>
               ...
               $(document).ready(function() {
                  $('#reservation').daterangepicker(null, function(start, end, label) {
					...
					<form name="admin_list_sea" class="form-search form-horizontal" method="post" action="/yoga/index.php/Admin/News/knows_back">
		...
		<option value="ms_author" <?php if($keytype == 'ms_author'): ?>selected<?php endif; ?>>按发布人</option>
							...
							<div class="col-xs-12 col-sm-3 btn-sespan">
							  ...
							  <div class="input-group-btn">
								<a href="/yoga/index.php/Admin/News/knows_back">
							...
							<div class="row">
							    <div class="col-xs-12">
										<div>
                                        <form id="alldel" name="alldel" method="post" action="<?php echo U('knows_back_alldel');?>" >
...
<td>
		<div class="hidden-sm hidden-xs action-buttons">
			<a class="red" href="javascript:;" onclick="return backopen(<?php echo ($v["ms_id"]); ?>,<?php echo I('p',1);?>);" title="还原">
																	<i class="ace-icon fa fa-check bigger-130"></i>																</a>
																<a class="red" href="javascript:;" onclick="return del(<?php echo ($v["ms_id"]); ?>,<?php echo I('p',1);?>);" title="删除">
...
function del(id,p){
	    ...
	    layer.close(index);
	    window.location.href="/yoga/index.php/Admin/News/knows_back_del/ms_id/"+id+"/p/"+p+"";//重新获取当前页面，删除后返回当前页码
	});
}
function backopen(id,p){
		layer.confirm('你确定要还原知识到知识列表吗？', {icon: 3}, function(index){
	    layer.close(index);
	    window.location.href="/yoga/index.php/Admin/News/knows_back_open/n_id/"+id+"/p/"+p+"";//重新获取当前页面，删除后返回当前页码
	});
}
