...
<?php if(is_array($data)): foreach($data as $key=>$v): ?><li class="<?php if(CONTROLLER_NAME == $v['name']): ?>active open<?php endif; ?>"><!--open代表打开状态-->
      ...
      <script type="text/javascript" src="/yoga/Public/sldate/daterangepicker.js"></script>
               ...
               $(document).ready(function() {
                  $('#reservation').daterangepicker(null, function(start, end, label) {
					...
					<form name="admin_list_sea" class="form-search form-horizontal" method="post" action="/yoga/Admin/News/news_list">
	...
	<select name="opentype_time">
	  ...
	  <option value="1" <?php if($opentype_time == 1): ?>selected<?php endif; ?>>已过期</option>
	  <option value="2" <?php if($opentype_time == 2): ?>selected<?php endif; ?>>未过期</option>
	  </select>
	  ...
	  <option value="1" <?php if($opentype_check == 2): ?>selected<?php endif; ?> >开启</option>
							...
							<div class="col-xs-12 col-sm-3 btn-sespan">
							  ...
							  <div class="input-group-btn">
								<a href="/yoga/Admin/News/news_list">
										...
										<div class="table-responsive">
											<table width="100%" class="table table-striped table-bordered table-hover" id="dynamic-table">
                                                ...
                                                <?php if(is_array($activedata)): foreach($activedata as $key=>$v): ?><tr>
														...
														<td><a class="item-sketch" href="" title="<?php echo ($v["ad_title"]); ?>" data-value='<?php echo ($v["ad_id"]); ?>'><?php echo (subtext($v["ad_title"],25)); ?></a>【<?php echo ($v["admin_username"]); ?>】</td>
														...
														<td><?php echo (date("Y-m-d H:i:s",$v["ad_starttime"])); ?></td>
														<td>
														<?php if($v[ad_open] == 1): ?><a class="red" href="javascript:;" onclick="return stateyes(<?php echo ($v["ad_id"]); ?>);" title="已开启">
														<div id="zt<?php echo ($v["ad_id"]); ?>"><button class="btn btn-minier btn-yellow">开启</button></div>
														</a>
														<?php else: ?>
														<a class="red" href="javascript:;" onclick="return stateyes(<?php echo ($v["ad_id"]); ?>);" title="已禁用">
														<div id="zt<?php echo ($v["ad_id"]); ?>"><button class="btn btn-minier btn-danger">禁用</button></div>
														</a><?php endif; ?>
														</td>

														<td align="center"><?php if($v["ad_valid"] == 2): ?><span>未过期</span><?php else: ?><span style="color:red;">已过期</span><?php endif; ?></td>
														<td>
															<div class="hidden-sm hidden-xs action-buttons">
																<a class="red" href="javascript:;" onclick="return backopen(<?php echo ($v["ad_id"]); ?>,<?php echo I('p',1);?>);" title="还原">
																	<i class="ace-icon fa fa-check bigger-130"></i>																</a>
																<a class="red" href="javascript:;" onclick="return del(<?php echo ($v["ad_id"]); ?>,<?php echo I('p',1);?>);" title="删除">
																	<i class="ace-icon fa fa-close bigger-130"></i>
																	<a  href="<?php echo U('active_register_list',array('ad_id'=>$v['ad_id']));?>"  title="报名查看">
				<i class="ace-icon fa  " style="font-size:11px;text-decoration:underline">报名查看</i>
								...
								<div class="row item-detail">
									...
									<div class="col-md-12">活动详情预览:<a id="contents" href=""></a></div>
...
function del(id,p){
	    ...
	    layer.close(index);
	    $.get("<?php echo U('News/news_back_del');?>",
	    		{n_id:id,p:p},
	    		function(data){
	    			if(data.status==1){
...
function backopen(id,p){
		layer.confirm('你确定要还原活动到活动列表吗？', {icon: 3}, function(index){
	    layer.close(index);
	    window.location.href="/yoga/Admin/News/news_back_open/n_id/"+id+"/p/"+p+"";//重新获取当前页面，删除后返回当前页码
	});
}
