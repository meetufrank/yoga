    ...
    <?php if(is_array($dataa)): foreach($dataa as $key=>$j): ?><li class="<?php if(($_SESSION['se'] == $j['id'])): ?>active<?php endif; ?>">
	  ...
	  $(function() {
		  $('.item-sketch').click(function(e) {
				...
				$.ajax(
					{
					url: '<?php echo U("class_register_id");?>',
					data: {
						id: $(this).data("value")
					},
					dataType: 'json',
      ...
      <script type="text/javascript" src="/yoga/Public/sldate/daterangepicker.js"></script>
               ...
               $(document).ready(function() {
                  $('#reservation').daterangepicker(null, function(start, end, label) {
					...
					<form name="admin_list_sea" class="form-search form-horizontal" method="post" action="/yoga/Admin/News/class_register_list">
							<div class="row maintop">
							<div class="col-xs-12 col-sm-3">
							<input type="hidden" value="<?php echo ($_GET['mc_id']); ?>" name="register_id">
	<select name="keytype">
		...
		<option value="ar_author" <?php if($keytype == 'ar_author'): ?>selected<?php endif; ?>>按报名用户名</option>
							...
							<div class="col-xs-12 col-sm-3 btn-sespan">
							  ...
							  <div class="input-group-btn">
								<a href="/yoga/Admin/News/class_register_list?mc_id=<?php echo ($_GET['mc_id']); ?>">
							...
							<div class="row">
							    <div class="col-xs-12">
										<div>
                                        <form id="alldel" name="alldel" method="post" action="<?php echo U('class_register_alldel');?>" >
										<input name="p" id="p" value="<?php echo I('p',1);?>" type="hidden" />
										<input name="alldel_id" id="d" value="<?php echo ($_GET['mc_id']); ?>" type="hidden" />
										<div class="table-responsive">
											<table width="100%" class="table table-striped table-bordered table-hover" id="dynamic-table">
                                                ...
                                                <?php if(is_array($register)): foreach($register as $key=>$v): ?><tr>
														<td align="center">
														<label class="pos-rel">
															<input name='cr_id[]' id="navid" class="ace"  type='checkbox' value='<?php echo ($v["cr_id"]); ?>'>
														<span class="lbl"></span>
														</label>
														</td>

														<td><a class="item-sketch" href="" title="<?php echo ($v["cr_name"]); ?>" data-value='<?php echo ($v["cr_id"]); ?>'><?php echo ($v["cr_name"]); ?></a></td>

														<td>

													<span ><?php echo ($v["cr_tel"]); ?></span>


														</td>
														<td><?php echo ($v["cr_email"]); ?></td>
														<td><?php if($v['cr_sex'] == 1 ): ?>男<?php else: ?>女<?php endif; ?></td>
														<td><?php echo ($v["cr_addtime"]); ?></td>
														<td><?php echo ($v["age"]); ?></td>
														<td>
														<?php if($v['cr_open'] == 1): ?><a class="red" href="javascript:;" onclick="return stateyes(<?php echo ($v["cr_id"]); ?>,<?php echo ($v["cr_classid"]); ?>);" title="已通过">
														<div id="zt<?php echo ($v["cr_id"]); ?>"><button class="btn btn-minier btn-yellow">已通过</button></div>
														</a>
														<?php else: ?>
														<a class="red" href="javascript:;" onclick="return stateyes(<?php echo ($v["cr_id"]); ?>,<?php echo ($v["cr_classid"]); ?>);" title="未审核">
														<div id="zt<?php echo ($v["cr_id"]); ?>"><button class="btn btn-minier btn-danger">未审核</button></div>
														</a><?php endif; ?>
														</td>
														<td><?php if($v['cr_more'] == ''): ?>未参加过<?php else: echo ($v["cr_more"]); endif; ?></td>


														<td>
		<div class="hidden-sm hidden-xs action-buttons">
			<a class="green" href="<?php echo U('class_register_listedit',array('cr_id'=>$v['cr_id']));?>" title="修改">
				<i class="ace-icon fa fa-pencil bigger-130"></i>
			</a>
			<a class="red" href="javascript:;" onclick="return del(<?php echo ($v["cr_id"]); ?>,<?php echo I('p',1);?>,<?php echo ($v["cr_classid"]); ?>);" title="回收站">
...
function del(id,p,ids){
	    ...
	    layer.close(index);
	    window.location.href="/yoga/Admin/News/class_register_del/cr_id/"+id+"/p/"+p+"/mc_id/"+ids+"";//重新获取当前页面，删除后返回当前页码
	});
}
...
function stateyes(val,clval){
		  $.post('<?php echo U("classregister_list_state");?>',
		  {x:val,id:clval},
	function(data){
	var $v=val;
		if(data.status){
