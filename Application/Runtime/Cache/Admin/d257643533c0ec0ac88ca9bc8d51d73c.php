...
<script type="text/javascript" src="/yoga/Public/assets/js/region.js"></script>
						<div class="row">
							<div class="col-xs-12">
								<form class="form-horizontal" name="form0" id="form0"  method="post" action="<?php echo U('knows_runedit');?>"  enctype="multipart/form-data">


									<div class="space-4"></div>

                                	<div class="form-group">
										<label class="col-sm-2 control-label no-padding-right" for="form-field-1"> 知识名称：  </label>
										<div class="col-sm-10">
										<input type="hidden" name="ms_id" value="<?php echo ($knowsdata["ms_id"]); ?>"/>
											<input type="text" name="title" id="title"  placeholder="必填：知识标题"  class="col-xs-10 col-sm-6" value="<?php echo ($knowsdata["ms_title"]); ?>"/>
									...
									<div class="form-group">
										...
										<a href="javascript:;" class="file">
                                          <input type="file" name="pic_one[]" id="file0" />
											...
											<span class="lbl">&nbsp;&nbsp;<img <?php if($knowsdata.ms_photo): ?>src="/yoga/Public<?php echo ($knowsdata["ms_photo"]); ?>"<?php else: ?> src="/yoga/Public/img/no_img.jpg"<?php endif; ?> width="100" height="70" id="img0" ></span>&nbsp;&nbsp;<a href="javascript:;" onClick="return backpic('/yoga/Public/img/no_img.jpg');" title="还原修改前的图片" class="file">
                                            撤销上传
										</a>
    ...
    <script src="/yoga/Public/ppy/js/fileinput_locale_zh.js" type="text/javascript"></script>
									...
									<div class="form-group">
										...
										<div class="col-sm-10" style="padding-top:5px;">
                                            <input name="open" id="open" value="1" class="ace ace-switch ace-switch-4 btn-flat" type="checkbox" <?php if($knowsdata["ms_open"] == 1): ?>checked="checked"<?php endif; ?> />
											<span class="lbl">&nbsp;&nbsp;注意：知识需要开启用户才能看到发布信息</span>
										</div>
									</div>
                                    ...
                                    <div class="form-group">
										...
										<div class="col-sm-10" style="padding-top:5px;">
                                            <input name="top" id="top" value="1" class="ace ace-switch ace-switch-4 btn-flat" type="checkbox" <?php if($knowsdata["ms_top"] == 1): ?>checked="checked"<?php endif; ?> />
									...
									<div class="form-group">
										<label class="col-sm-2 control-label no-padding-right" for="form-field-1"> 知识简介： </label>
										<div class="col-sm-9">
										<input type="hidden" name="ms_content" value="<?php echo ($knowsdata["ms_content"]); ?>"/>
											<input type="text" name="say" id="news_scontent" class="col-xs-10 col-sm-10" value="<?php echo ($knowsdata["ms_say"]); ?>" maxlength="50" />
									...
									<div class="form-group">
										<label class="col-sm-2 control-label no-padding-right" for="form-field-1"> 知识主内容 </label>
										<div class="col-sm-10">
											...
											<textarea name="content" rows="100%" style="width:100%" id="myEditor"><?php echo ($knowsdata["ms_content"]); ?></textarea>
											<script type="text/javascript">
...
$(function(){
	...
	function checkForm(){
		...
		if( '' == $.trim($('#title').val())){
			layer.alert('知识标题不能为空', {icon: 6}, function(index){
 			layer.close(index);
			$('#title').focus();
			});
			return false;
		...
		if( '' == $.trim($('#news_scontent').val())){
			layer.alert('知识简介不能为空', {icon: 6}, function(index){
 			layer.close(index);
			$('#news_scontent').focus();
			});
			return false;
